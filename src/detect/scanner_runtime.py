...
def run_one_polymarket_complement_cycle(
    ...
    from src.ingest.polymarket_orderbook import (
        PROCESSED_PATH as POLYMARKET_ORDERBOOKS_OUTPUT_PATH,
    ...
    try:
        run_polymarket_orderbook_ingestion(limit_markets=limit_markets)
        ...
        if flags:
            write_flags_jsonl(flags, source="polymarket_live_complement")
    except Exception as exc:
        error_msg = str(exc)
    ...
    return ComplementRunEvent(
        ...
        output_path=str(POLYMARKET_ORDERBOOKS_OUTPUT_PATH) if orderbook_ingestion_ok else None,
        error=error_msg,
    )
