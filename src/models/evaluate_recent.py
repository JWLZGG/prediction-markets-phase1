...
PLOT_PATH = Path("reports/calibration_recent_24h.png")
...
def evaluate_recent() -> None:
    ...
    X = df[
    ["category", "duration_hours", "market_implied_prob", "log_volume", "log_liquidity"]].copy()
    ...
    market_brier = brier_score_loss(y_test, X_test["market_implied_prob"].astype(float).values)
    ...
    plt.title("Calibration / Reliability Plot (Recent 24h)")
    plt.legend()
    plt.tight_layout()
